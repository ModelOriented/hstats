...
ylogx <- function(y, x) {
  ...
  ok <- x != 0
  if (!(all(y[!ok] == 0))) {
    stop("ylog(x) undefined if x = 0 and y != 0")
  }
  out[ok] <- y[ok] * log(x[ok])
  out
}
...
loss_poisson <- function(actual, predicted) {
  ...
  2 * (ylogx(y = actual, x = actual / predicted) - (actual - predicted))
}
...
loss_logloss <- function(actual, predicted) {
  ...
  -ylogx(y = actual, x = predicted) - ylogx(y = 1 - actual, x = 1 - predicted)
}
...
loss_mlogloss <- function(actual, predicted) {
  ...
  -rowSums(ylogx(y = actual, x = predicted))
}
