...
- `hstats()`, `partial_dep()`, `ice()` now also work for factor predictions. Each level is represented by its one-hot-encoded values.
- Much faster one-hot-encoding, written by Mathias Ambühl.
