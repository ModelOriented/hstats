...
$$
A value of 0 means there are no interaction effects at all. A similar measure using accumulated local effects is discussed in \cite{molnar2020}.
...
$$
	\textrm{PDI}_{j}^2 = \frac{\frac{1}{n} \sum_{i = 1}^n\big[F(\mathbf{x}_i) - \hat F_{\setminus j}(\mathbf{x}_{i\setminus j})\big]^2}{\frac{1}{n} \sum_{i = 1}^n\big[F(\mathbf{x}_i)\big]^2}.
$$
