...
test_that("results reacts to `m_rep` (univariate)", {
  set.seed(1L)
  s2 <- perm_importance(fit, X = iris[-1L], y = y, m_rep = 100L)
...
test_that("perm_importance() accepts functions as losses (univariate)", {
  set.seed(1L)
  s2 <- perm_importance(fit, X = iris[-1L], y = y, m_rep = 2L, loss = loss_gamma)
  set.seed(1L)
  s3 <- perm_importance(fit, X = iris[-1L], y = y, m_rep = 2L, loss = "gamma")
  expect_equal(s2, s3)
})
...
test_that("perm_importance() reacts to `m_rep` (multivariate)", {
  set.seed(1L)
  s2 <- perm_importance(fit, X = iris[3:5], y = y, m_rep = 1L)
  expect_false(identical(s1$M, s2$M))
...
test_that("perm_importance() reacts to `loss` (multivariate)", {
  set.seed(1L)
  s2 <- perm_importance(fit, X = iris[3:5], y = y, m_rep = 2L, loss = "gamma")
  expect_false(identical(s1, s2))
})

test_that("perm_importance() accepts functions as losses (multivariate)", {
  set.seed(1L)
  s2 <- perm_importance(fit, X = iris[3:5], y = y, m_rep = 2L, loss = loss_gamma)
  
  set.seed(1L)
  s3 <- perm_importance(fit, X = iris[3:5], y = y, m_rep = 2L, loss = "gamma")
  expect_equal(s2, s3)
})
