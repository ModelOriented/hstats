...
test_that("Non-additive models show interactions > 0 (one interaction)", {
  ...
  out <- H2_overall(inter, plot = FALSE)
...
test_that("Non-additive models show interactions > 0 (two interactions)", {
  ...
  out <- H2_overall(inter, plot = FALSE, sort = FALSE, normalize = FALSE, squared = FALSE)
  expect_equal(
    rownames(out[out > 0, , drop = FALSE]), 
    c("Petal.Length", "Petal.Width", "Species")
  )
  
  out <- H2_pairwise(inter, plot = FALSE, sort = FALSE, normalize = FALSE, squared = FALSE)
  expect_equal(
    rownames(out[out > 0, , drop = FALSE]), 
...
# )
# H2_pairwise(inter, squared = FALSE, sort = FALSE)
